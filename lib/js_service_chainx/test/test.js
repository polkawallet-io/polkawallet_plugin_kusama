...
async function runSettingsTest() {
  ...
  const endpoint = "wss://mainnet.chainx.org/ws";
...
async function runKeyringTest() {
  ...
  expect(acc2.address, "5SN1P8jw3KqmA3JSo7WJj6fB8iyYEBbkW57jgz8iAGK3eX92");
  ...
  expect(acc3.address, "5UFxizY1yumujQz5CyXvgpNyHwNNaY2rQunFgN5RpVBfZ4P7");
  ...
  expect(encoded[0][acc.pubKey], "13kzksW5uLyt8ejW3659gXWVc8TcYUGJfX4xh6wywdhayhCW");
...
async function runAccountTest() {
  ...
  const testAddr = "5TLM4wEBMEJysMSj5TY5BKv3K4aaE1XRE5mv6FUt6NzT1PFT";
  ...
  const addr2 = "5TLM4wEBMEJysMSj5TY5BKv3K4aaE1XRE5mv6FUt6NzT1PFT";
  const info = await account.getAccountIndex(api, [addr2]);
  expect(info[0].accountId.toString(), addr2);
  expect(info[0].identity.display, "ExxiBox");
  expect(info[0].identity.web, "http://ExxiBox.com");
